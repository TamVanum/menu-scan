...
exports.EditarAlimento = (req, res) =>{
    ...
    conexion.query('UPDATE alimento SET ?',[{categoria_id_fk: categoria, nombre_alimento:nombre, foto_alimento:"", precio: precio, descripcion:descripcion, estado_alimento_id_fk:1}, id], (error, results)=>{
        if(error){
            console.log(error);
        }else{
            res.redirect('/');
